...
Due to the limitations of Bedrock edition, player heads need to be pre-compiled instead of being created on the fly. This basically means that you need to create each head from scratch before they appear in game. <ins>Because if this, you'll need to build the addon on a (windows) computer before you can use it!</ins> This repository contains a python script that builds the pack for you. You'll need to have some experience in asset creation to use this addon.
...
- Place your head assets (models, sounds, textures) in the user assets folder. I added template files in the user assets folder to help you get started with creating your own heads. Feel free to remove them if you desire.
- Edit the HeadsToCreate.csv and add the heads you wish to create.
- <ins>Before you move on, there are a few guidlines you need to follow in the csv file:</ins>
  - Make sure each entry in the name column matches the name of the player it belongs to, otherwise that player will not drop their head!
  - If there are any spaces in the player's name, for example "man dude", make sure to replace each of them with an underscore in the name column.
  - If the texture file (in user assets) has the same file name as the entry in the name column, you do not need to define the texture in the texture column.
  - The sound and model column should not be left empty.
  - If you'd like the block to have a different display name you can chose to do so here!
